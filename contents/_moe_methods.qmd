...
\begin{align*}
    \begin{aligned}
        ...
        & & & \int_0^T \ell (x(t),u) \diff t , \\%
        \textrm{subject to}
        & & & M(q) \diff \dot{q} + h(x; \psi, \theta)\diff t - \diff R  = 0,\\%
...
* Training procedure:
  ...
  \begin{gather*}
    ...
    M(q) \diff \dot{q} + h(x; \psi, \theta) \diff t - \diff R  = 0 \text{ (Moreau's time stepping)}
  \end{gather*}
