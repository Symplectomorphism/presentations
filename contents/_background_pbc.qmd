...
## Lemma (Khalil, 2002)
- The origin is __stable__ if $\Sigma$ is passive
- The origin is __asymptotically stable__ if $\Sigma$ is output strictly passive
- The origin is __globally asymptotically stable__ if $\Sigma$ is output strictly passive and $\mathcal{H}$ is radially unbounded
:::
...
Main idea --- search for functions $H_d$ and $h_d$ such that $u(x) = u_{es} + u_{di}$ renders the closed-loop system passive
...
Choose $u = u_{es} + u_{di}$ and $H_d$ satisfying $\dot{H}_{d} \leq y^\top u_{di}$ 
:::
