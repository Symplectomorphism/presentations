...
- If underactuated, $H_d$ must satisfy $G$ is not invertible
  * Choice of $u_{es}$ must satisfy **nonlinear PDE**
  * Quadratic $V_d$ most likely not viable
...
$$u = u_{es} + u_{di} = \left(G^{\top} G \right)^{-1} \left( \nabla_q H  - \nabla_q H_d \right)  - K_D G^\top \nabla_p H_d $$
:::

::: {.fragment .fade-in  fragment-index=3}
$$u = u_{es} + u_{di} = - \left(G^{\top} G \right)^{-1} \nabla_q H_d^{\theta}  - K_D G^\top \nabla_p H_d^{\theta} $$
:::
...
\end{bmatrix} u^{\theta}
\\
&& u^\theta &= \left(G^{\top} G \right)^{-1} \left( - \nabla_q H_d^{\theta}  - K_D G^\top \nabla_p H_d^{\theta} \right)
\end{aligned}
$$
:::
...
::: {layout="[80,100]" layout-valign="center"}
![](assets/neuralpbc/033.svg)
...
$$l_{\text{set}}(x) = \underset{t}{\inf} \left\{ \lVert a-b \rVert : a \in \gamma(t), b \in \mathcal{S}\right\}$$
...
<div>
* Defined on a transversal section along $\gamma^\star$
* $\tau$ a surrogate for time
...
::: {.fragment .fade-in fragment-index=1}
1. Simulating the system under application of $u^\theta$ (initially poorly-trained)
2. Collect samples from the regions of state-space visited by $u^\theta$
:::

[^dagger]: Ross, S., Gordon, G. J., & Bagnell, J. A. (2011). No-regret reductions for imitation learning and structured prediction. In In AISTATS.

## <span style="font-variant:small-caps;">NeuralPbc</span> Algorithm
<!-- ###################################################################### -->
