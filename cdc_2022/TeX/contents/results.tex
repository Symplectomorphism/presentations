...
% In simulation, the performance metric is chosen as the accumulated expenditure
% of control action $\|u^\theta\|$ along the trajectories of the closed-loop
% system.
% %
% If the control law $u^{\theta}$ fails to bring the system inside the region of
% attraction of LQR, i.e., failing to stabilize $x^\star$, the performance metric
% is defined as $\infty$.
% %
% Figure~\ref{fig:compare_uRu} shows that the expenditure of
% control action by the \textsc{NeuralPbc} controller is much lower (2 orders of
% magnitude) than that of the classical \textsc{IdaPbc} control law.
% \begin{figure}[tb]
%     \centering
%     \includegraphics[width=\linewidth]{figures/compare_uRu_neuralpbc_idapbc.eps}
%     \caption{
%         %
%         Comparison of accumulated control effort $\|u^\theta\|$ required to
%         swing up the IWP, computed from simulated trajectories starting from
%         each $(q_1, \dot{q_1})$ grid. From left to right: \textsc{NeuralPBC} and
%         classical \textsc{IdaPbc}.
%         %
%     }
%     \label{fig:compare_uRu}
% \end{figure}
